...
func (p *Plugin) updateGlobalCommands() {
	...
	for _, v := range CommandSystem.Root.Commands {
		if cmd := p.yagCommandToSlashCommand(v); cmd != nil {
			logger.Infof("%s is a global slash command, enabled by default: %v", cmd.Name, *cmd.DefaultPermission)
			result = append(result, cmd)
		}
	}

	for _, v := range slashCommandsContainers {
		logger.Infof("%s is a slash command container, enabled by default: %v", v.container.Names[0], v.defaultPermissions)
