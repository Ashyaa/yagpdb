...
func (roleMenuL) LoadEditingOption(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRoleMenu interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		if !queries.IsNil(object.EditingOptionID) {
			args[object.EditingOptionID] = struct{}{}
		}
		...
		for _, obj := range slice {
			...
			if !queries.IsNil(obj.EditingOptionID) {
				args[obj.EditingOptionID] = struct{}{}
			}
	...
	query := NewQuery(
		...
		qm.WhereIn(`role_menu_options.id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (roleMenuL) LoadNextRoleCommand(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRoleMenu interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		if !queries.IsNil(object.NextRoleCommandID) {
			args[object.NextRoleCommandID] = struct{}{}
		}
		...
		for _, obj := range slice {
			...
			if !queries.IsNil(obj.NextRoleCommandID) {
				args[obj.NextRoleCommandID] = struct{}{}
			}
	...
	query := NewQuery(
		...
		qm.WhereIn(`role_commands.id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (roleMenuL) LoadRoleGroup(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRoleMenu interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		if !queries.IsNil(object.RoleGroupID) {
			args[object.RoleGroupID] = struct{}{}
		}
	...
	query := NewQuery(
		qm.From(`role_groups`),
		qm.WhereIn(`role_groups.id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (roleMenuL) LoadRoleMenuOptions(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRoleMenu interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`role_menu_options`),
		qm.WhereIn(`role_menu_options.role_menu_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *RoleMenu) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns, opts...)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *RoleMenu) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(roleMenuPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert role_menus, could not build conflict column list")
			}

			conflict = make([]string, len(roleMenuPrimaryKeyColumns))
			copy(conflict, roleMenuPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"role_menus\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o *RoleMenu) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return RoleMenuExists(ctx, exec, o.MessageID)
}
